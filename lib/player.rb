...
module HungerGames
	class Player
		...
		#	Given the round number, your current food, your current reputation, the extra hunt threshold m, and an array of other player reputations,
		#	create an array (indexed just like other_player_reputations) of 'h' or 's' indicating whether or not you want to hunt with the player at that index
		#	Note that other_player_reputations is shuffled each time, so you cannot maintain state using the indices to identify other players
		def hunt_choices(round_number, current_food, current_reputation, m, other_player_reputations)
			raise NotImplementedError
		end

		#	After hunting each round, you players get an opportunity to examine how things went. the food_earnings array is indexed just like other_player_reputations
		#	and contains the net result of each hunt, depending on how successfully you and your 'partner' hunted.
		def hunt_outcomes(food_earnings)
			raise NotImplementedError
		end

		#	At the very end of the round, each player is notified about the overall group performance and the extra hunt bounty.
